...
export function SessionReviewWorkspace({ ownerId, params, sessionId, review }: { ownerId: string; params: IdParams; sessionId: string; review: SessionReviewData }) {
  ...
  return <>
    ...
    <section className="card settings-content-card session-review-card" aria-label="Transcript editor">
      ...
      {transcript?.state === "complete" && segments.length ? <div className="transcript-segments">{segments.map((segment, index) => <article id={transcriptSegmentAnchor(segment.start, segment.end)} className={segment.deleted ? "transcript-segment deleted" : "transcript-segment"} key={`${segment.start}-${segment.end}`}><div className="transcript-time">{formatTimestamp(segment.start)}–{formatTimestamp(segment.end)}</div><textarea aria-label={`Transcript segment ${index + 1}`} value={segment.text} disabled={segment.deleted || busy} onChange={(event) => setSegments((current) => current.map((item, itemIndex) => itemIndex === index ? { ...item, text: event.target.value } : item))} /><label className="transcript-delete"><input type="checkbox" checked={Boolean(segment.deleted)} disabled={busy} onChange={(event) => setSegments((current) => current.map((item, itemIndex) => itemIndex === index ? { ...item, deleted: event.target.checked } : item))} />Hide segment</label></article>)}</div> : <div className="transcript-empty">{transcript?.state === "failed" ? "Transcript unavailable after provider retries. Audio evidence is preserved for recovery." : "Timestamped transcript segments will appear here when processing completes."}</div>}
    </section>
  </>;
}
