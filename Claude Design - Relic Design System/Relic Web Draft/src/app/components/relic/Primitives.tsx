...
export function I({ n, size = 18, sw = 1.6, fill = "none", cls = "", style }: IProps) {
  return (
    <svg
      className={"i " + cls}
      width={size}
...
export function Avatar({ name = "", initials, size = 32, cls = "", style }: AvatarProps) {
  const text = initials || name.split(/\s+/).map(w => w[0]).slice(0, 2).join("").toUpperCase() || "?";
  const avatarStyle: React.CSSProperties = {
    background: tintFor(name || text),
    width: size,
    height: size,
    fontSize: Math.round(size * 0.38),
    ...style,
  };
  return <div className={"avatar " + cls} style={avatarStyle}>{text}</div>;
}
...
export function AvatarStack({ people = [], max = 4, more }: AvatarStackProps) {
  const shown = people.slice(0, max);
  const extra = more != null ? more : people.length - shown.length;
  return (
    <div style={{ display: "flex" }}>
      {shown.map((p, i) => (
        <Avatar key={i} name={p} style={{ marginLeft: i > 0 ? -8 : 0, border: "1.5px solid var(--bg-2)" }} />
      ))}
      {extra > 0 && (
        <div style={{ width: 32, height: 32, borderRadius: "50%", marginLeft: -8, background: "var(--bg-3)", border: "1.5px solid var(--bg-2)", display: "grid", placeItems: "center", fontFamily: "var(--font-mono)", fontSize: 10, color: "var(--stone-700)" }}>
          +{extra}
        </div>
      )}
    </div>
...
export function Dot({ kind = "", pulse }: DotProps) {
...
export function Chip({ tone = "stone", dot, dotKind, children, outline }: ChipProps) {
  if (outline) return (
    <span className={"chip-outline " + outline}>
      {dot && <Dot kind={dotKind || tone} />}{children}
    </span>
  );
  return (
    <span className={"chip " + tone}>
      {dot && <Dot kind={dotKind || ""} />}{children}
    </span>
  );
}
...
export function Meter({ value = 0, max = 100, cls = "" }: MeterProps) {
  return (
...
export function SecLabel({ icon, count, children, action }: SecLabelProps) {
  return (
