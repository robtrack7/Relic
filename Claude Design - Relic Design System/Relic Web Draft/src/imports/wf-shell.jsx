// wf-shell.jsx — Relic Shell: TopBar, SideNav, Loom, Overlays
// Exports to window: Shell, TopBar, SideNav, LoomCarriage,
//   SearchPalette, CreateMenu, SwitcherDropdown, ShellOverlayDemo
...
function NotificationsDropdown({ reviewCount, onClose }) {
  const items = [
  ...
  const confColor = (c) => c === "high" ? "var(--verdigris)" : c === "med" ? "var(--amber)" : "var(--rust)";
  const confBg = (c) => c === "high" ? "var(--verdigris-dim)" : c === "med" ? "var(--amber-dim)" : "var(--rust-dim)";
  return (
    <div style={{ position: "absolute", top: "calc(100% + 7px)", right: 0, width: 308, background: "var(--bg-2)", border: "1px solid var(--border-1)", borderRadius: "var(--radius-lg)", boxShadow: "var(--shadow-elevated)", zIndex: 300, overflow: "hidden" }}>
      ...
      <div key={i} style={{ padding: "8px 14px", borderTop: i > 0 ? "1px solid var(--border-2)" : "none", display: "flex", alignItems: "flex-start", gap: 9, cursor: "pointer" }}>
          ...
          <span style={{ fontFamily: "var(--font-mono)", fontSize: 7, padding: "2px 5px", borderRadius: "var(--radius-sm)", flexShrink: 0, background: confBg(item.conf), color: confColor(item.conf) }}>{item.conf}</span>
        </div>
      )}
      {extra > 0 &&
      ...
      <div style={{ padding: "10px 14px", borderTop: "1px solid var(--border-2)" }}>
        <button className="btn btn-ink btn-sm" style={{ width: "100%", justifyContent: "center", gap: 7 }}
        onClick={() => {window.selectWireframeScreen?.("rv-queue");onClose();}}>
...
function TopBar({ active = "home", collapsed, onToggleNav, pill = "none", reviewCount = 0, showUsage = false, usageWarn = false, onSearch, onCreate, onSwitcher }) {
  const pillLabels = {
  ...
  const p = pillLabels[pill] || pillLabels.none;
  ...
  return (
    <div className="topbar">
      <div className="topbar-brand">
        ...
        <button className="nav-toggle" onClick={onToggleNav} title="Toggle nav">
      ...
      <div className="switchers">
        <div className="switcher">
          ...
          <button className="switcher-btn" onClick={() => onSwitcher?.("workspace")}>
        ...
        <div className="switcher">
          ...
          <button className="switcher-btn" onClick={() => onSwitcher?.("world")}>
        ...
        <div className="switcher">
          ...
          <button className="switcher-btn" onClick={() => onSwitcher?.("saga")}>
      ...
      <div className="topbar-right">
        ...
        {pill !== "none" &&
        <button className={"sess-pill " + p.cls}>
        ...
        <div style={{ position: "relative" }} ref={notifRef}>
          ...
          style={notifOpen ? { background: "var(--bg-3)", color: "var(--ink)" } : {}}>
            <I n="bell" size={16} />
...
function CreateDropdown({ onClose }) {
  ...
  return (
    ...
    style={{ position: "absolute", left: 0, top: "calc(100% - 3px)", zIndex: 200, margin: 0, width: "100%", borderTopLeftRadius: 0, borderTopRightRadius: 0, borderTop: "none" }}
    onClick={(e) => e.stopPropagation()}>
      {items.map((item, i) => item ?
      <div key={i} className="cm-item" onClick={onClose}>
...
function SideNav({ active = "home", collapsed, initialCreateOpen = false }) {
  const [createOpen, setCreateOpen] = React.useState(initialCreateOpen);
  const createWrapRef = React.useRef(null);
  const allItems = [...NAV_ITEMS, ...NAV_BOTTOM];
  return (
    <nav className="sidenav">
      {!collapsed && <div className="sidenav-label">Navigation</div>}
      {NAV_ITEMS.map((item) =>
      <div key={item.id} className={"nav-item" + (active === item.id ? " is-active" : "")}>
          <span className="nav-ico"><I n={item.icon} size={16} /></span>
          {!collapsed && <span className="nav-text">{item.label}</span>}
          {!collapsed && item.count > 0 && <span className="nav-count">{item.count}</span>}
        </div>
      )}
      {!collapsed ?
      <div ref={createWrapRef} style={{ position: "relative", margin: "10px 8px 4px" }}>
            <button className="btn btn-ink" style={{ fontSize: 12, gap: 7, justifyContent: "flex-start", padding: "8px 12px", width: "100%", position: "relative", zIndex: 201 }} onClick={() => setCreateOpen((o) => !o)}>
              <I n="plus" size={14} /> Create
            </button>
            {createOpen && <CreateDropdown onClose={() => setCreateOpen(false)} />}
          </div> :
      <div style={{ position: "relative" }}>
            <div className="nav-item" style={{ justifyContent: "center", backgroundColor: "rgb(56, 52, 46)", color: "rgb(228, 221, 212)" }} onClick={() => setCreateOpen((o) => !o)}>
              <I n="plus" size={16} />
            </div>
            {createOpen && <CreateDropdown onClose={() => setCreateOpen(false)} iconOnly />}
          </div>}
...
function LoomCarriage({ expanded = true, mode = "ask", onExpand }) {
  const modes = ["Ask", "Prep", "Live"];
  if (!expanded) return (
    <div className="loom-carriage col" onClick={onExpand}
    title="Open The Loom"
    ...
    onMouseLeave={(e) => e.currentTarget.style.background = ""}>
      <div style={{ marginTop: 16, color: "var(--amber)", width: 36, height: 36, display: "grid", placeItems: "center", borderRadius: "var(--radius-md)", border: "1px solid rgba(184,112,42,0.3)", background: "rgba(184,112,42,0.06)" }}>
        <I n="spark" size={16} />
      </div>
  ...
  return (
    <div className="loom-carriage exp">
      <div className="loom-head">
        ...
        <button className="loom-close-btn"><I n="chevRight" size={13} /></button>
      </div>
...
function LoomLiveContent() {
  return <>
    ...
    <div style={{ display: "flex", gap: 5, flexWrap: "wrap" }}>
      ...
      <button key={i} style={{ fontSize: 10, fontFamily: "var(--font-mono)", letterSpacing: "0.08em", padding: "4px 8px", border: "1px solid var(--stage-border)", borderRadius: "var(--radius-sm)", color: "var(--stage-fg-2)", background: "var(--stage-card)" }}>{s}</button>
      )}
    </div>
  </>;
}
  ...
  return (
    <div className="scrim" onClick={onClose}>
      <div className="search-palette" onClick={(e) => e.stopPropagation()}>
        ...
        <div key={i} className={"sp-row " + cls}>
...
function CreateMenu({ onClose, anchorRect }) {
  const items = [
  { icon: "users", label: "Character (NPC)" },
  { icon: "map", label: "Place or location" },
  { icon: "anchor", label: "Faction" },
  ...
  const top = anchorRect ? anchorRect.bottom + 4 : 340;
  const left = anchorRect ? anchorRect.right + 6 : 260;
  return (
    <div className="scrim" style={{ background: "transparent" }} onClick={onClose}>
      <div className="create-menu" style={{ position: "fixed", top, left, margin: 0 }} onClick={(e) => e.stopPropagation()}>
        {items.map((item, i) => item ?
...
function SwitcherDropdown({ which = "saga", onClose }) {
  const options = {
  ...
  return (
    <div className="scrim" style={{ alignItems: "flex-start", paddingTop: "var(--shell-topbar-h)", paddingLeft: 216 }} onClick={onClose}>
      <div className="switcher-dd" onClick={(e) => e.stopPropagation()}>
        ...
        <div key={i} className={"sd-item" + (item.cur ? " current" : "")}>
            {item.cur && <div style={{ display: "flex", alignItems: "center", gap: 5 }}>
              <Dot kind="active" />
              <span className="sd-name">{item.name}</span>
            </div>}
            {!item.cur && <div className="sd-name">{item.name}</div>}
...
function Shell({ active = "home", navCollapsed = false, loomExpanded = true, loomMode = "ask", pill = "none", reviewCount = 0, showUsage = false, usageWarn = false, overlay = null, onOverlayClose, children, stageMode = false }) {
  const [navCol, setNavCol] = React.useState(navCollapsed);
  const [loomExp, setLoomExp] = React.useState(loomExpanded);
  const [ov, setOv] = React.useState(overlay);
  const [createRect, setCreateRect] = React.useState(null);

  React.useEffect(() => {setNavCol(navCollapsed);}, [navCollapsed]);
  React.useEffect(() => {setLoomExp(loomExpanded);}, [loomExpanded]);
  React.useEffect(() => {setOv(overlay);}, [overlay]);
  ...
  return (
    <div className={"relic"} data-nav={navCol ? "rail" : "full"} style={{ height: "100%", overflow: "hidden" }}>
      <TopBar
        ...
        onCreate={() => setOv("create")}
        onSwitcher={(w) => setOv("sw-" + w)} />
      ...
      <div className="body-row">
        ...
        <LoomCarriage expanded={loomExp} mode={loomMode} onExpand={() => setLoomExp(true)} />
      </div>
      {ov === "search" && <SearchPalette onClose={() => setOv(null)} />}
      {(ov === "sw-workspace" || ov === "sw-world" || ov === "sw-saga") &&
      <SwitcherDropdown which={ov.replace("sw-", "")} onClose={() => setOv(null)} />}
    </div>);

}

// ═══ Shell overlay demos (standalone screens) ════════════════

function ShellStateDemo({ variant }) {
  const configs = {
  ...
  const cfg = configs[variant] || configs["shell-default"];
  return (
...
Object.assign(window, { Shell, TopBar, SideNav, LoomCarriage, SearchPalette, CreateMenu, SwitcherDropdown, ShellStateDemo });
