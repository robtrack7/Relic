...
**Packet E8 knowledge-proposal patch (August 2026).** The existing Loom action ledger may materialize a confirmed Saga-scoped Character, Place, Faction, Artifact, Thread, or Note create/update proposal as exactly one pending `drafts` row with its originating answer run, action receipt, target version, frozen evidence, and a Saga-scoped GM-instruction source. Confirmation creates no canon or embedding job; the ordinary Approval Queue remains the only record commit boundary. Eligible World evidence linked to a Saga draft is authorized by the draft's active Workspace/World/Saga root rather than rejected because the source itself has `scope='world'`. Confirmed relationship and Thread state/objective actions reuse the existing scoped mutation RPCs and their version/source/audit behavior. Loom record proposals cannot mutate World canon in MVP.
