...
**Approved E8 implementation contract (2026-08-04):** extend the active registry from six to twelve actions with `propose_record_create@1.0.0`, `propose_record_update@1.0.0`, `add_relationship@1.0.0`, `remove_relationship@1.0.0`, `set_thread_state@1.0.0`, and `mutate_thread_objective@1.0.0`. The first two accept strict field-allowlisted Character, Place, Faction, Artifact, Thread, or Note payloads generated inside the already-metered `answer_saga_question@1.4.0` turn. Their separate zero-credit confirmation creates exactly one Saga-scoped pending Approval Queue draft, a bounded GM-instruction source, and frozen current-turn evidence links. It never commits canon; Approval Queue remains the only record commit boundary. Record proposals cannot target World canon in MVP.

Relationship actions identify endpoints only through current-turn source IDs. Loom relationship mutation is limited to current-Saga edges, so two World-only endpoints are rejected. Removal resolves one exact current relationship and freezes its edge version; duplicate/ambiguous/missing edges fail safely. Thread state and objective actions require a Saga Thread evidence source, freeze the Thread version, and reuse `update_thread_details` / `mutate_thread_objective`; objective text is resolved server-side to one stable objective ID, with duplicate or stale text rejected. These four actions show `0 additional credits · Canon change`, require explicit inline confirmation, and reuse the existing source/audit semantics and embedding triggers rather than adding Loom-specific mutation tables.

E8 also repairs the pre-existing Approval Queue source-health mismatch for Saga drafts citing eligible World evidence: authorization roots at the current Saga draft and accepts linked current-World sources, while sibling-Saga sources remain denied. `gm_instruction`, `existing_entity`, and eligible World source contexts become inspectable rather than mislabeled as broken/unsupported. Confirmation and exact replay must not duplicate drafts, edges, Thread effects, receipts, charges, audits, or embedding jobs. A successful later record approval enqueues one current content identity; proposal confirmation, rejection, dismissal, conflict, and relationship-only changes enqueue none.

Implementation order inside E8: canonical schema/task/retrieval/approval/cost/UX patch; Approval Queue source-boundary repair; six action contracts and `answer_saga_question@1.4.0` validation; source-bound target/diff materialization; idempotent proposal writer; existing relationship/Thread handler reuse; safe action projections/cards; focused adversarial and embedding-lifecycle tests; clean migration, full suites, production build, four-viewport successful-proposal plus stale-conflict browser proof, fresh-process recovery, and packet closeout. No paid provider call is implied.

**Complete at the deterministic gate (2026-08-04):** the active registry now contains twelve actions. Strict source-bound Character, Place, Faction, Artifact, Thread, and Note create/update proposals confirm into exactly one pending Approval Queue draft at zero additional credits; relationship and Thread state/objective actions require explicit inline GM confirmation and reuse the existing scoped writers. World-canon record mutation and cross-Saga targets fail closed. E8 also repairs Approval Queue inspection and health for eligible World evidence linked to a Saga draft. Proposal confirmation, replay, rejection, dismissal, relationship-only change, and stale conflict do not enqueue content embeddings; only a later approved record commit does. Clean replay, 992 pgTAP assertions, 86 script/runtime tests, 146 web tests, TypeScript, production build, four responsive viewports, targeted Review handoff, fresh-process persistence, and authenticated stale-evidence conflict proof pass. No paid provider call was made. The immediate packet is E9.
...
**Approved E9 implementation contract (2026-08-04):** extend the registry from twelve to sixteen actions with `create_session@1.0.0`, `open_session_workflow@1.0.0`, `start_prep_task@1.0.0`, and `retry_session_transcription@1.0.0`, while reusing `navigate_surface@1.0.0` for Approval Queue navigation and adding deterministic active-Workshop resume as an `open_session_workflow`-class navigation result. Obvious create/open/status/retry/Prep/Workshop commands are planned and materialized in the database without an `answer_saga_question` call, query embedding, or credit. Less exact conversational requests may still propose the same registered actions through `answer_saga_question@1.5.0`; the database independently resolves and revalidates every target.

`create_session` is a zero-credit reversible working-state confirmation that reuses `public.create_session` and returns the existing full Prep route. `open_session_workflow` is a free read/navigation action whose server-derived lifecycle projection routes planned/Ready Sessions to Prep, live or undo-window Sessions to Stage, and ended Sessions to Review with bounded transcription/pipeline status. `retry_session_transcription` is an explicit zero-credit retry that reuses the existing finalized-audio/failed-job guard and never reuploads audio or starts synthesis during Stage.

`start_prep_task` supports the six E4 entry tasks only: briefing (3), full Prep (10), scene beats (1), Thread complication (1), NPC candidates (1), and Quick Stub fleshing (1). Its card freezes the current editable Session version, exact source-bound target where required, registered task version, and displayed credit estimate before confirmation. Confirmation creates one idempotent handoff to the existing `prep-ai-submit` preflight/evidence/runner path; it does not execute a second Loom answer call or add a confirmation charge. The resulting proposal remains pending on the existing Prep surface. Text/pin/Thread application continues through local merge plus optimistic autosave, Quick Stub acceptance continues to the Approval Queue, and an NPC candidate still requires its separate reviewed 3-credit `draft_entity_from_prompt` follow-up. The Loom links to these owning review controls instead of bulk-applying output and bypassing local-draft recovery.

Workshop resume is current-user and Workspace-scoped, returns only the latest in-progress workshop, and never starts or charges a planning/scaffold task. Stage remains silent: workflow mutation and Prep generation are unavailable once a Session is started/live/ending/ended, while navigation, bounded status, and the documented manual paths remain available. Implementation order inside E9: active spec patch; lifecycle/workshop helpers; four registry contracts and `answer_saga_question@1.5.0` validation; deterministic command planning without auto-accepting confirmable actions; idempotent Prep dispatch receipt/finalization; safe workflow projections/cards; adversarial cost/isolation/replay tests; clean migration, full suites, production build, responsive browser success plus failure recovery, fresh-process persistence, and packet closeout. No paid provider call is implied.

**Complete at the deterministic gate (2026-08-04):** the sixteen-action registry, exact zero-provider workflow planner, free confirmed Session creation, lifecycle-aware Prep/Stage/Review and Workshop navigation, explicit transcription retry, and six separately priced E4 Prep handoffs are implemented. Prep confirmation uses one private receipt and the existing E4 request/run path, charges only the selected task, leaves generated output pending on Prep, and rejects stale or Stage-live targets before dispatch. Authenticated browser proof also exposed and repaired a pre-existing hardened-access mismatch in `guide-submit`: retry and source resolution now use narrow service-only worker RPCs instead of denied direct public-table reads. Clean replay, 1,027 pgTAP assertions, 88 script/runtime tests, 149 web tests, TypeScript, production build, authenticated success/conflict coverage, fresh-process persistence, and four responsive viewports pass. No paid provider call was made. The immediate packet is E10.

### E10 - Destructive Actions, Bounded Plans, and Phase Gate

**Approved E10 implementation contract (2026-08-04):** extend the registry from sixteen to nineteen actions with `archive_record@1.0.0`, `restore_record@1.0.0`, and `prepare_hard_delete@1.0.0`, and advance the compatibility answer task to `answer_saga_question@1.6.0`. The lifecycle actions are Saga-record-only. Exact typed lifecycle commands resolve by current scoped type/name in the database and dispatch no answer task or query embedding; ambiguous names have no effect. Provider-originated lifecycle actions must bind a current-turn source. Archive and restore freeze the record version, require an explicit zero-credit Loom confirmation, and reuse `archive_entity` / `restore_entity` so their manual source, canon audit, and embedding eligibility behavior remains singular.

`prepare_hard_delete` is not a delete executor. Its explicit confirmation writes one read/navigation receipt and exposes the owning archived-record route plus current dependency summary. The record page then requires its existing irreversible-action checkbox, exact current name, and final submit; only that page can call `hard_delete_entity`, which still rechecks archived state, references, and version. World-canon lifecycle mutation, hard-delete submission, and Saga deletion are absent from the Loom registry.

A provider response may contain a plan of two to five action previews only when every step is from the zero-additional-credit confirmable deterministic allowlist. Every preview carries `plan: {step, depends_on}`; steps are contiguous and dependencies are unique earlier step numbers. Read/navigation actions, `draft_entity`, `start_prep_task`, `prepare_hard_delete`, and Saga deletion are ineligible. The database validates the complete plan independently, freezes each server-owned effect/cost and plan relation, exposes only the first step, and unlocks one next step after an accepted receipt. Each step has its own review and confirmation; there is no Approve All. Conflict, denial, dismissal, or failure stops every later step with zero effects and no provider continuation. Implementation order: active spec patch; three lifecycle registry/materializer/review paths; protected record-page handoff; dual Edge/database plan validation; sequential stop semantics and safe projections; component/pgTAP/runtime tests; combined deterministic browser/retrieval/accessibility/cost gate; fresh-process persistence; clean full Phase E regression; then separately request authorization for any hosted task-family gate.
...
**Complete at the deterministic gate (2026-08-04):** the registry now contains nineteen actions and `answer_saga_question@1.6.0`. Exact Saga-record archive, restore, and permanent-delete preparation resolve without an answer task, query embedding, or AI charge; archive/restore reuse the existing version-checked lifecycle writers, while delete preparation opens only the archived record's existing checkbox plus exact-name panel and never calls the delete executor. Two-to-five-step provider plans independently pass Edge and database validation, expose only the next eligible zero-additional-credit confirmable step, require separate confirmations, and stop every later step after dismissal, conflict, denial, or failure. Clean migration replay, all 37 database files / 1,061 pgTAP assertions, 91 script/runtime tests, 152 web tests, TypeScript, production build, repository verification, database lint with no E10-local finding, and the authenticated lifecycle/plan/conflict browser proof pass. The 30-query retrieval evaluation retains recall@5 1.0, MRR 0.96, zero isolation failures, and lexical fallback success. No hosted or paid provider call was made. E5.1-E10 implementation is therefore complete at the deterministic boundary; the only remaining Phase E gate is the separately authorized hosted task-family smoke described below.
...
The gate therefore represents 28 product credits. Each logical task permits its normal initial completion plus at most one schema-repair completion; the global provider-completion ceiling is 12. No generic retry, provider continuation, query embedding, transcription, synthesis, action confirmation, canon commit, archive, restore, or delete execution is allowed. A temporary isolated LiteLLM proxy exposes only the three declared aliases, runs at concurrency one, disables prompt/raw-payload logging, and enforces the remaining `max_budget: 3.49` over a two-hour window after the conservative cumulative prior-attempt reserve. The wrapper additionally stops when the task ledger estimate plus that reserve exceeds `$4.00`. The GM explicitly authorized this revised provider envelope on 2026-08-05 with `PHASE_E_HOSTED_SMOKE_APPROVED`. This is not an all-in infrastructure cap: the temporary Fly build and machine minutes may be billed separately by Fly, cannot be bounded by LiteLLM, and must be disclosed before authorization. The wrapper destroys the app in `finally` to minimize that uncapped infrastructure exposure.
...
**Hosted accounting repair (2026-08-05):** every successful provider completion is recorded in a private, payload-free, idempotent per-call ledger before JSON parsing, schema validation, or the one allowed repair. Run-level token and cost fields are cumulative projections of that ledger, so an invalid first response, a successful repair, or a later delivery failure cannot erase billable work. The worker uses the proxy-reported cost when available and otherwise computes a conservative upper-bound estimate from returned token usage and the pinned GPT-5.6 list-price table; missing usage makes the cost evidence incomplete and stops the smoke before another call. Provider-call starts and completed responses are bounded separately. Exact redelivery cannot add either. GPT-5.6 reasoning effort is server-pinned rather than inherited from the provider default: `relic-fast` uses `none`, while `relic-balanced` and `relic-deep` use `low` for these strict structured-output tasks. This is the hosted baseline to evaluate; any later increase must be justified by representative quality evidence and an updated cost contract.

**Successful-repair projection repair (2026-08-05):** the passing hosted gate correctly counted all eight provider completions and their cumulative cost, but exposed that `repair_attempts` was persisted only on terminal failure. A successfully repaired scaffold or entity draft therefore showed two provider completions with zero repair attempts. Preserve the private per-call ledger as the authoritative cost/call record, and additionally pass the bounded repair count into the provider-output checkpoint so successful repairs project `repair_attempts = 1`. Keep the old checkpoint signature during rolling deploys, expose the new overload only to `service_role`, and prove initial-only, successful-repair, exact-redelivery, and terminal-failure behavior with pgTAP. This is an accounting projection fix; it does not require another paid provider run and does not alter the already-passing hosted safety evidence.

All pre-success 2026-08-05 provider attempts are now conservatively reserved at `$0.50`. The first two task-family attempts measured `$0.00888125` and `$0.00647225`; the third completed the Luna workshop once and then used the initial plus repair allowance for the deep Sol scaffold before failing validation. The third attempt's complete ledger was intentionally removed with the synthetic fixture, and its full cost value was beyond the wrapper's bounded failure-detail window, so the reserve is raised rather than inferred downward. The remaining isolated proxy budget for this authorization is therefore `$3.49`; the combined reserve plus new proxy budget remains below the `$4.00` provider stop.

**Hosted execution status (2026-08-05): passed; Phase E complete.** After aligning all six provider-facing nested contracts without weakening their validators or raising model output ceilings, the authorized staging gate completed `plan_saga_workshop`, `scaffold_saga`, `regenerate_saga_scaffold_section`, `answer_saga_question`, `generate_session_prep`, and `draft_entity_from_prompt`. All six runs completed with six usage events and eight provider completions against the pinned Luna/Sol/Terra models; the conservative successful-run cost estimate was `$0.153853625`, complete for every call. The scaffold and entity draft each required one bounded schema-repair completion; no task exceeded its repair allowance or the twelve-completion ceiling. Exact replay added no provider call, usage event, or cost. Proof state contained exactly one source-linked pending non-canon entity draft and zero canon audit rows, action intents, query embeddings, embedding events, outside-source matches, unsafe telemetry matches, or raw payload keys. Cleanup then removed all fixture rows and the draft, restored the prior staging provider configuration, JWT verification, both schedules, and the prior ignored local provider credential, destroyed the temporary Fly app, and left both queues idle with no temporary secret/smoke files. The subsequent successful-repair projection fix is applied as migration `20260805220000`; `ai-task-runner` v89 is active with JWT verification enabled, the new checkpoint overload is `service_role`-only, and clean replay plus all 38 database files / 1,084 pgTAP assertions pass. The Phase E goal and Milestone E gate are closed; the next delivery packet is F1.
