...
The gate therefore represents 28 product credits. Each logical task permits its normal initial completion plus at most one schema-repair completion; the global provider-completion ceiling is 12. No generic retry, provider continuation, query embedding, transcription, synthesis, action confirmation, canon commit, archive, restore, or delete execution is allowed. A temporary isolated LiteLLM proxy exposes only the three declared aliases, runs at concurrency one, disables prompt/raw-payload logging, and enforces the remaining `max_budget: 3.74` over a two-hour window after the conservative prior-probe reserve. The wrapper additionally stops when the task ledger estimate plus that reserve exceeds `$4.00`. The GM explicitly authorized this revised provider envelope on 2026-08-05 with `PHASE_E_HOSTED_SMOKE_APPROVED`. This is not an all-in infrastructure cap: the temporary Fly build and machine minutes may be billed separately by Fly, cannot be bounded by LiteLLM, and must be disclosed before authorization. The wrapper destroys the app in `finally` to minimize that uncapped infrastructure exposure.
...
The failed 2026-08-05 Luna probe is conservatively reserved at `$0.25` even though its bounded request could not approach that amount at the published Luna rates. The remaining isolated proxy budget for this authorization is therefore `$3.74`; the combined reserve plus new proxy budget remains below the `$4.00` provider stop.

**Hosted execution status (2026-08-05): blocked on provider credential scope.** Staging is current through E10 plus the accounting repair, the revised runner is deployed with gateway JWT verification restored, queues are empty, schedules are active, and every synthetic row and temporary Fly app was removed. The hosted worker reached OpenAI but received `provider_rejected` before a completion. A separate 16-token compatibility probe against the same protected key returned `401` with `missing_scope`, proving that the key lacks Chat Completions write permission. No successful provider completion, provider token usage, or model cost was recorded by either attempt. The next run requires a replacement project key authorized to write `/v1/chat/completions`; do not widen, create, or rotate that credential implicitly. Once the key is replaced in the protected local provider state, rerun the same bounded gate with the `$0.25` conservative reserve and `$3.74` remaining proxy budget.
