...
**Packet E9 workflow-orchestration patch (August 2026).** The Loom reuses Session, Prep, post-session, Approval Queue, and Workshop owner contracts rather than introducing model-selected handlers. Four registered actions cover Session creation, lifecycle-aware workflow navigation/status, separately metered Prep task handoff, and transcription retry. Exact commands complete planning without a provider or embedding; confirmable deterministic actions remain pending until GM review rather than being auto-accepted. Prep confirmation freezes the editable Session version and task-specific source-bound target, writes one private idempotent dispatch receipt, and calls the existing internal-token `prep-ai-submit` boundary; authenticated finalization records only accepted/quota/failed handoff state. The generated result is reviewed and applied on Prep through its existing local merge/autosave or Approval Queue path. Stage status prevents Prep dispatch while live, and Workshop resume is a read-only current-user/Workspace projection.

**Packet E8 knowledge-proposal patch (August 2026).** `answer_saga_question@1.4.0` may return six additional manifest actions: Saga record create/update proposals, relationship add/remove, and Thread state/objective mutation. The database independently validates field allowlists, source-bound targets, Saga-only write scope, relationship endpoints, objective identity, current evidence, and optimistic record/edge versions. Record confirmation writes one pending Approval Queue draft plus frozen source links and an idempotent Loom receipt, never canon. Direct relationship/Thread confirmation invokes the existing authenticated mutation functions from the locked action transaction; it does not introduce model-selected RPCs or parallel write semantics. Exact replay cannot duplicate a draft, relationship effect, Thread audit, usage charge, or embedding enqueue.
...
**Packet E5.2 conversational workshop patch (August 2026).** New Saga uses two explicit provider boundaries: one light `plan_saga_workshop` call creates a validated three-to-five-question interview plan and initial outline, then server-owned deterministic turn RPCs autosave answers and advance that finite plan without provider calls. `Draft it now` performs a separate heavy quota preflight, freezes the full normalized GM conversation, and dispatches `scaffold_saga@1.2.0`. Targeted regeneration is a separately disclosed standard task whose versioned result may replace only the requested review section when the base review version still matches. All actions use private idempotency receipts; navigation, save-and-leave, answer persistence, review edits, confirmation, and exact replay are unmetered.
...
**Packet E3 local-provider isolation correction (July 2026).** `supabase functions serve --env-file` is treated as the authoritative Edge environment. The local helper now composes a temporary file that defaults AI, embedding, and transcription to deterministic modes and omits all live provider endpoints/credentials; explicit live opt-in is required. This follows an E3 synthetic-only incident in which process overrides failed to replace live values and four LiteLLM requests were made before detection. Safe telemetry, cleanup, and a clean-reset deterministic rerun are recorded in the E3 session log.
