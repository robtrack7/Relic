...
- Phase E passed its deterministic and hosted gates on 2026-08-05. [[74 - Phase F Trust Data and Operations Plan]] owns the delivered F1-F3 trust/control-plane lane: deterministic settings, retention-aware Loom recovery, user-owned exports with safe exclusions, and private-content-free notification delivery. Phase F passed its clean milestone gate on 2026-08-05. [[75 - Phase G Release Quality and Integrated MVP Plan]] owns the open release lane, beginning with G0 hosted prerequisite closeout before G1 feature closure.
...
- The July 2026 D5 patch replaces the Import Inbox placeholder with provider-independent paste and strict UTF-8 text/Markdown intake. The August 2026 Phase G patch adds trusted text-bearing campaign/World PDF extraction plus explicit source enrollment into the existing Loom workshop. Upload/extraction alone remains excluded from retrieval/embeddings/AI/proposals/canon. Rulebook RAG and scanned-PDF OCR/vision remain V1; `.docx` remains deferred.
...
- The August 2026 E9 patch upgrades synthesis to `answer_saga_question@1.5.0` and expands the registry to sixteen with Session creation, lifecycle/Workshop navigation, six bounded Prep handoffs, and transcription retry. Exact workflow commands bypass answer-task quota and query embeddings. Prep confirmation dispatches only the selected existing E4 task after disclosed cost and version review, never charges confirmation, and leaves output pending on Prep; stale or Stage-live targets fail before dispatch. Hardened Edge access now uses narrow service-only replay/source-resolution RPCs rather than direct public-table reads.
- The August 2026 E10 patch upgrades synthesis to `answer_saga_question@1.6.0` and expands the registry to nineteen with Saga-record archive, restore, and protected permanent-delete preparation. Exact lifecycle commands bypass the provider; archive/restore reuse existing optimistic writers; delete preparation can only open the archived record's checkbox plus exact-name panel and never exposes `hard_delete_entity` to The Loom. Provider plans are limited to two-to-five independently validated zero-additional-credit confirmable actions, unlock sequentially, and stop all later steps after dismissal, conflict, denial, or failure. E5.1-E10 are complete at the deterministic boundary; the separately authorized hosted task-family smoke remains the sole open Milestone E gate.
...
- The current backend has foundation schema/RLS/RPC scaffolding plus completed backend modules for access boundaries, canon provenance, Approval Queue commits, E1 deterministic/hosted embedding delivery and hybrid retrieval, manual Session Prep/Stage data, usage/quota metering, Storage/audio/transcript contracts, shared job/Edge runtime, AI task runtime, notifications, exports, stale review scanning, and cleanup finalization. E2 additionally proves the shared hosted transcription, embedding, light/deep AI, scheduling, observability, retry/replay, metering, and isolation boundaries in synthetic staging. E3-E10 now provide the persistent Loom conversation, non-canon Prep/workshop generation, typed action kernel, adaptive retrieval, source-bound knowledge proposals, reviewed workflow handoffs, protected lifecycle operations, and sequential bounded plans. The six-family Phase E hosted harness is locally proven at zero calls with strict queue, task, model, repair, cost, telemetry, cleanup, and rollback guards; executing that paid staging smoke, real notification/export providers, and production promotion remain separate gates.
