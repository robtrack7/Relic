...
- The July 2026 E5 in-progress patch supersedes the disabled New Saga AI cards and any interpretation that provider completion may create a World, Saga, entity, relationship, Thread, Session, or Prep row. `scaffold_saga@1.1.0` completes only into a recoverable workshop draft over immutable GM-input/eligible-World evidence. GM edits/removals use optimistic review saves, and `commit_saga_workshop` is the separate explicit transaction that publishes the selected hierarchy and canon exactly once. The first bounded live delivery failed strict validation safely and does not satisfy the E5 completion gate; the deterministic workflow remains usable and Start Blank remains the full manual fallback.
