...
Packet E10 adds Saga-record archive/restore cards and protected hard-delete preparation. Archive and restore show the exact record, current lifecycle state, reversible effect, zero-credit cost, and explicit confirmation before the existing version-checked writer runs. Hard-delete preparation has its own explicit Loom confirmation, then opens the archived record's existing permanent-delete panel; the GM must still acknowledge irreversibility and type the exact current name there. The Loom never accepts the name or submits deletion. Multi-action plans show two to five numbered steps, each server-owned effect and cost, and explicit earlier-step dependencies. Only the next eligible step can be confirmed. Conflict, denial, failure, or dismissal labels every later step `Plan stopped`; the GM may start a new turn, but no automatic continuation or Approve All control exists.
