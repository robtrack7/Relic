...
MVP accepts JPEG, PNG, or WebP objects up to 5 MiB, 8,192 pixels on either edge, and 32 megapixels total. It rejects SVG, animated formats, malformed files, MIME/magic mismatch, and active/polyglot content. Objects live in a private `attachments` bucket under the hierarchy prefix.
...
**2026-08-05 Library metadata checkpoint:** normal Library detail now edits descriptive status and up to twelve normalized tags for Character, Place, Faction, Artifact, and Thread records through the existing 800 ms/blur optimistic boundary. Status remains separate from canon state; Notes retain their separate `note_type` contract. The server revalidates metadata, deduplicates tags, preserves source/audit provenance and stale-version conflicts, and refreshes the embedding job when tags, status, or GM notes change so metadata-only edits cannot leave retrieval stale. The migration is deployed to locked staging; clean replay plus 1,209 database assertions, 157 web tests, and 105 runtime tests pass. Upload/metadata editing makes no direct provider call. G1 now continues with private image attachments and GM-authored Stage references.

**2026-08-05 private-image checkpoint:** bounded private static-image attachments are complete locally and on locked Supabase staging. Registration derives one exact private Storage path for a current-Saga Character, Place, Faction, Artifact, Thread, or Note; the JWT-authenticated Edge boundary validates JPEG/PNG/WebP structure, magic/MIME agreement, byte/dimension limits, animation exclusion, hashes, and duplicate content before `ready`. The Library surface supports GM-authored title, required alt text, optional atmosphere description, 60-second private viewing, optimistic metadata edits, and object-first deletion. An explicit Loom handoff freezes only authored text, safe format/dimensions, and target provenance with `GM-authored description; image not inspected`; pixels, paths, hashes, signed URLs, automatic AI, embedding, drafts, and canon writes remain excluded. Six real local and hosted runtime cases pass (three static formats plus malformed, oversized-dimension, and MIME-mismatch rejection), including view/delete lifecycle. Clean replay plus 1,240 database assertions, 159 web tests, and 113 script/runtime tests pass with zero provider calls. G1 now continues only with GM-authored Stage references before the twin real-Loom journeys.

**2026-08-05 G1 closure checkpoint:** The Stage `GM Screen` now derives exclusively from Note records pinned during Session Prep, searches the GM-authored title/summary/body, excludes other pinned record kinds, and directs an empty state back to Prep. All built-in D&D-like rules examples, system-rule search claims, and d20 advantage/disadvantage controls are removed. Dice remains a system-neutral pool/modifier convenience roller and explicitly tells the GM that Relic does not interpret the result. Full verification passes with 1,240 database assertions, 161 web tests, 113 script/runtime tests, TypeScript, production build, repository checks, and zero provider calls. G1 is complete; G2 twin real-Loom journeys are next once the G0 hosted-console/web deployment prerequisites are available.
