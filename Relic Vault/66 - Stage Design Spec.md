...
The Create overlay maps NPC, Location, Item, Thread, and Faction to GM-authored canon stubs with manual source/audit provenance and a live-session source link. Create → Note remains evidence rather than a stub: it writes a session-linked `quick_capture` note whose title is the GM's chosen name and whose body is the short note. Successful writes invalidate the Library read so a refresh must show the record with the correct type and stub semantics. While a write is pending, Create stays single-submit; validation or permission failure leaves the entered fields recoverable.

When a d20 is in the Dice pool, Normal, Advantage, and Disadvantage are explicit modes. Pinning the pool preserves the selected mode, and a pinned roll records that mode in the same scoped session dice contract used by an unpinned roll. Reopening Dice starts a fresh pool in Normal mode; pin state is session-local presentation, while completed rolls remain session evidence.

For the current web MVP, session-active Threads render as a compact, read-only strip in the primary Stage column between Agenda and search. Each item shows its name, current state, a short summary fallback, and objective count; an explicit empty state appears when prep carried no Threads forward. The strip uses the already scoped `get_session_active_threads` read and introduces no Stage mutation, navigation detour, or sixth action.

The A3 header pass is intentionally restrained. Keep the same Stage identity, session number, lifecycle/recording/upload states, elapsed time, Start/Go live control, Loom access, Sanctum exit, and profile destination, but present `The Stage` and its session number as quiet identity/context text rather than adjacent configuration-shaped pills. `GM Screen` keeps its MVP name. Responsive collapse may hide the Stage wordmark before hiding lifecycle or recovery state.

Browser `MediaRecorder` capture is isolated behind a recording adapter. Completed ~30-second chunks are persisted in IndexedDB before direct authenticated Storage upload and idempotent chunk registration. The Stage shows queued, uploading, failed, and recovered states; app-level recovery resumes saved chunks after reconnect/reload. Note contains the recording-only Mark Moment secondary action so the five-button rail remains unchanged. Complete-set transcription eligibility and scheduled undo finalization use the existing backend boundary plus the scoped audio-finalization marker. Provider delivery is still incomplete and must not be implied by these evidence-path states. The same rule applies to Relic Guide: live/prep Loom presentation must not imply that the AI task entrypoint is wired when it is not.
