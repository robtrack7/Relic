...
**Packet E10 plan and lifecycle metering patch (August 2026).** Exact archive, restore, and protected-delete preparation cost zero credits and create no query embedding or answer task. Confirming, dismissing, replaying, version-conflicting, or manually completing an owning lifecycle action is also free. A multi-action plan is limited to five zero-additional-credit deterministic steps; it cannot contain `draft_entity`, `start_prep_task`, any other downstream provider task, read-only filler, hard-delete preparation, or Saga deletion. The originating conversational answer costs at most its ordinary one light credit, confirmation never repeats it, and later steps stop without an automatic repair/replan call. The Phase E deterministic gate therefore has zero billable provider attempts; hosted task-family validation remains a separate, newly disclosed authorization.

**Packet E9 workflow metering patch (August 2026).** Exact Session creation/navigation/status, transcription retry, Approval Queue navigation, and active-Workshop resume cost zero AI credits and create neither an answer task nor a query embedding. A Prep action displays and freezes its registered cost before confirmation: briefing 3, full Prep 10, scene beats/Thread complication/NPC candidates/Quick Stub fleshing 1. Confirmation and dispatch handoff add zero credits; the existing Prep task creates at most one usage event for its idempotent request. Review, local merge/autosave, rejection/dismissal, source inspection, Prep navigation, Quick Stub Approval Queue commit, and an exact dispatch replay are free. Drafting a selected NPC remains a new, separately confirmed 3-credit task; it is never hidden in the earlier 1-credit candidate request.

**Packet E7 adaptive-read metering patch (August 2026).** Recognized open/list/source/provenance/navigation and structured record/relationship/Thread/Session reads dispatch neither a text-model task nor a query-embedding request and cost zero credits. Exact grounding and literal FTS also avoid query embeddings; a text-model `answer_saga_question` call still costs one light credit when synthesis is actually required. Hybrid retrieval may add embedding provider cost only after deterministic, exact, structured, and lexical paths are insufficient. Ambiguous targets, missing provenance, stale or missing vectors, permission denial, sibling-Saga filtering, deterministic replay, and manual navigation charge nothing.

**Packet E8 knowledge-action metering patch (August 2026).** A record payload or mutation action proposed by `answer_saga_question@1.4.0` is part of that one-credit response; confirming, dismissing, creating the pending draft, approving or rejecting it, and executing confirmed relationship/Thread mutations are deterministic and cost zero additional AI credits. `draft_entity@1.0.0` remains the separate optional deep 3-credit task for substantial lore generation. Stale evidence, invalid fields, unavailable targets, duplicate relationships, approval conflicts, exact replay, and rejected/dismissed proposals make no provider request and no usage charge.
...
**Packet E5.2 workshop metering patch (August 2026).** Starting a conversational workshop dispatches one disclosed light/1-credit `plan_saga_workshop` task. Persisting answers, advancing its stored three-to-five-question plan, emerging-outline updates, `Save and leave`, review edits, and exact replay are deterministic and free. `Draft it now` separately discloses and preflights one heavy/10-credit `scaffold_saga` task. A confirmed targeted section regeneration is a separate standard/3-credit task and may replace only that section. The normal complete creation path therefore costs 11 credits before optional regeneration; no hidden per-message provider charge exists. Explicit discard cancels queued/retryable workshop runs before claim and charges nothing; once a call is already running, discard waits until its usable result or failure is recorded so paid work is not silently thrown away.
