...
const APPROVED_COST_CEILING_USD = 0.50;
...
const turnSpecs = [
  ...
  {
    question: "Offer a clearly labeled non-canon creative proposal for a complication involving the Amber Warden.",
    expected: "creative",
  },
  {
...
insert into public.sources (
  ...
  ('${IDs.characterSource}','${IDs.workspace}','${IDs.world}','${IDs.saga}','saga',
   'gm_instruction','character','${IDs.character}','Synthetic current-Saga evidence.'),
  ('${IDs.worldSource}','${IDs.workspace}','${IDs.world}',null,'world',
   'gm_instruction','place','${IDs.worldPlace}','Synthetic World evidence.'),
...
from internal.ai_task_runs where id='${response.body.run_id}';
  ...
  if (run.status !== "complete") {
    fail(`Guide turn ${index + 1} stopped safely in ${run.status}/${run.failure_category ?? "unknown"}.`);
  }
  return submission;
}
...
select jsonb_build_object(
  ...
  'ai_runs',(
    select jsonb_agg(jsonb_build_object(
      ...
      'source_versions_frozen',jsonb_object_length(allowed_source_versions)>0,
  ...
  'isolation',jsonb_build_object(
    ...
    'draft_writes',(select count(*) from public.drafts where workspace_id='${IDs.workspace}')
  ),
...
function validateEvidence(evidence) {
  ...
  if (!Array.isArray(evidence.ai_runs) || evidence.ai_runs.length !== 3
    || evidence.ai_runs.some((run) => run.status !== "complete"
      || run.task_name !== "answer_saga_question"
  ...
  if (evidence.ai_runs[0].conversation_context_items !== 0
    || evidence.ai_runs.slice(1).some((run) => run.conversation_context_items < 2)) {
  ...
  if (grounded.no_answer || grounded.citation_count < 1
    ...
    fail("The hosted grounded answer lacked a paragraph citation.");
  }
