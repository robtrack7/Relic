...
declare
  response uuid;
begin
  if export_id is null or to_regprocedure('internal.complete_export_job(uuid,text,bigint,text)') is null then
    return null;
  end if;

  execute 'select internal.complete_export_job($1,$2,$3,$4)'
    using export_id,
          'a1200000-0000-0000-0000-000000000001/a1300000-0000-0000-0000-000000000001/a1500000-0000-0000-0000-000000000001/' || export_id::text || '.zip',
          2048::bigint,
          repeat('a',64)
    into response;
  return response;
...
select has_function('internal', 'complete_export_job', array['uuid','text','bigint','text'], 'complete_export_job worker helper exists');
...
select has_function('public', 'claim_audio_cleanup_job_for_worker', array['text'], 'audio cleanup has a kind-specific claim boundary');
select has_function('public', 'claim_saga_cleanup_job_for_worker', array['text'], 'Saga/export cleanup has a kind-specific claim boundary');
select has_function('public', 'claim_export_job_for_worker', array['text'], 'export dispatcher has an exposed service-only claim boundary');
select has_function('public', 'claim_notification_job_for_worker', array['text'], 'notification dispatcher has an exposed service-only claim boundary');
...
update internal.export_jobs set include_audit=true where id=(select id from module10_export_ids);
create temp table module10_export_payload as select public.get_saga_export_payload_for_worker((select id from module10_export_ids)) as value;
select is((select value#>>'{payload,saga,name}' from module10_export_payload),'Module 10 Saga','worker projection contains the exact requested Saga');
select ok((select value#>'{payload,audit}' is not null from module10_export_payload),'audit option includes user-owned audit and Loom sections');
select ok((select position('provider_payload' in value::text)=0 from module10_export_payload),'export projection excludes private provider payload keys');
...
select ok((select response ->> 'storage_path' from module10_export_responses where label = 'download') like 'a1200000-0000-0000-0000-000000000001/%', 'completed export returns an authorized private signing target');
...
select is((select count(*) from cron.job where jobname in ('relic-dispatch-exports','relic-dispatch-cleanup-saga','relic-dispatch-cleanup-audio','relic-enqueue-expired-exports')),4::bigint,'export generation, expiry scan, and kind-specific cleanup schedules exist');
insert into internal.cleanup_jobs(id,workspace_id,world_id,saga_id,job_kind,payload,idempotency_key) values
 ('a1900000-0000-0000-0000-000000000002','a1200000-0000-0000-0000-000000000001','a1300000-0000-0000-0000-000000000001','a1500000-0000-0000-0000-000000000001','audio','{}','module10-audio-claim'),
 ('a1900000-0000-0000-0000-000000000003','a1200000-0000-0000-0000-000000000001','a1300000-0000-0000-0000-000000000001','a1500000-0000-0000-0000-000000000001','exports','{}','module10-export-claim');
select is(public.claim_audio_cleanup_job_for_worker('module10-audio-worker')->>'job_kind','audio','audio worker cannot claim export cleanup');
select is(public.claim_saga_cleanup_job_for_worker('module10-saga-worker')->>'job_kind','exports','Saga cleanup worker claims expired exports without consuming audio work');
