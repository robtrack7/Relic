...
AI task execution must start with its registered preflight boundary, then run through the internal Edge task runner. Worker-only wrappers expose the internal `ai_task_runs` ledger to service-role workers without granting browser execution. The five-argument `record_ai_task_output_for_worker(...)` result boundary also receives the final resolved model/provider. For `synthesize_session`, it revalidates the immutable source set and exact Workspace/World/Saga/Session ownership, then atomically writes one pending draft batch. For the Saga workshop, `create_saga_workshop(...)` performs the workspace-only 1-credit planning preflight, freezes GM input plus eligible World canon, and creates a recoverable run before a Saga exists. `answer_saga_workshop_question(...)` advances the bounded stored plan without provider work; `dispatch_saga_workshop_draft(...)` separately preflights the 10-credit scaffold; `request_saga_workshop_regeneration(...)` creates one 3-credit, review-version-bound section proposal; and `review_saga_workshop_regeneration(...)` merges only an explicitly accepted current proposal. `abandon_saga_workshop(...)` discards only uncommitted owner-scoped state. `get_workshop_evidence_for_worker(...)`, `set_workshop_state_for_worker(...)`, and `complete_workshop_for_worker(...)` can write only the workshop draft/proposal boundary. `commit_saga_workshop(...)` is a separate authenticated, optimistic, explicit GM transaction and is the only E5 path that may publish the reviewed structure to canon; normal entity and Session triggers then enqueue the committed records for embedding.
