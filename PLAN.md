...
**Milestone E gate:** the E5.1-E10 deterministic boundary is complete. Every implemented action has passed clean-replay scope, authority, idempotency, provenance, cost, canon, archive, and delete-negative coverage. The gate remains open only for a separately authorized staging smoke covering provider-dependent task families; Phase F stays closed until that authorization decision and resulting evidence are recorded.
...
Immediate decision gate: **Phase E hosted task-family smoke authorization**. E5.1-E10 are complete at the deterministic boundary, including the combined lifecycle, bounded-plan, retrieval, cost, and browser gate. Do not make a hosted or paid call until the user explicitly approves the environment, aliases/models, synthetic fixture, maximum attempts, dollar ceiling, exact command, cleanup, and rollback. After an authorized passing smoke, close Milestone E and begin **F1 — Settings, retention, and quota recovery**. D5 imported sources remain excluded unless a later explicit GM enrollment contract is added.
