...
- **E5.2 — Conversational Workshop and Scaffold Parity:** deliver the bounded three-to-five-turn Loom creation conversation, emerging outline, `Draft it now`, `Save and leave`, active scaffold cardinalities/fields, and versioned per-section regeneration without pre-commit product writes.
- **E6 — Loom Action Kernel:** typed server-owned registry, action cards, authority tiers, cost disclosure, target/version checks, idempotent execution receipts, and audit-safe failures. No arbitrary SQL/RPC selection.
...
Immediate next packet: **E5.1 — Profile-Aware Workshop Correctness**. Repair the profile/default/override, idempotency, and trusted task-input contract before expanding creation conversation or the Loom action registry. The existing failed E5 live proof does not authorize another paid call; any hosted smoke requires a fresh environment/model/fixture/attempt/cost/cleanup/rollback gate. D5 imported sources remain excluded unless a later explicit GM enrollment contract is added.
