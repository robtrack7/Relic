...
- **Phase E hosted-gate harness (permission-blocked 2026-08-05):** the gate pins six active contracts across `relic-fast`, `relic-balanced`, and `relic-deep` for 28 product credits, at most twelve provider completions, and a `$4.00` session stop. A `$0.25` conservative reserve for prior rejected probes leaves a `$3.74` one-concurrency temporary-proxy budget. The worker now records every successful completion in a private payload-free ledger before validation, accumulates repair/retry tokens and cost, falls back to conservative GPT-5.6 list-price estimates, captures safe failure evidence before cleanup, pins reasoning effort, and accepts only an equivalent provider-qualified resolved-model ID. Clean replay, 1,080 pgTAP assertions, 101 script/runtime tests, and zero-provider fixture proof pass. Staging reached the provider but the protected OpenAI key returned `401 missing_scope` for Chat Completions; therefore no successful completion or provider token/model cost occurred. Cleanup restored provider/JWT/schedules and removed every fixture and temporary app. Replace the protected project key with one that has `/v1/chat/completions` write permission before rerunning.

**Milestone E gate:** the E5.1-E10 deterministic boundary and hardened hosted harness are complete. Every implemented action has passed clean-replay scope, authority, idempotency, provenance, cost, canon, archive, and delete-negative coverage. Execution is authorized but environment-blocked because the protected OpenAI project key lacks Chat Completions write scope. Phase F stays closed until a correctly scoped replacement key produces the six-task hosted evidence.
...
Immediate decision gate: **replace the Phase E provider credential with a correctly scoped project key**. The GM authorized the `$4.00` hosted envelope, and the hardened staging gate reached OpenAI, but the protected key returned `401 missing_scope` before any completion. Install a project key with write permission for `/v1/chat/completions` in the existing protected local provider state; do not publish it or place it in tracked files. Then rerun the unchanged six-task gate with 28 product credits, at most twelve completions, a `$0.25` prior-probe reserve, and `$3.74` remaining proxy budget. After a passing smoke, close Milestone E and begin **F1 — Settings, retention, and quota recovery**. D5 imported sources remain excluded unless a later explicit GM enrollment contract is added.
