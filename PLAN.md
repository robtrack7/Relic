...
Immediate next packet: **E6 — Loom Action Kernel**. Add the typed server-owned capability registry, action-intent ledger/cards, authority and cost projection, optimistic targets, confirmation state, idempotent receipts, and audit-safe execution failures before expanding retrieval or knowledge-writing tools. E5.1-E5.2 are complete at the deterministic gate. No previous hosted-call authorization carries forward; any hosted smoke requires a fresh environment/model/fixture/attempt/cost/cleanup/rollback gate. D5 imported sources remain excluded unless a later explicit GM enrollment contract is added.
