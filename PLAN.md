...
- **Import and metadata slices complete (2026-08-05):** private Storage PDF upload, JWT-verified extraction, immutable hashes/provenance, safe lifecycle/UI recovery, and exact RPC/Storage boundaries are deployed. Ten local and hosted runtime cases pass, including a true PDF polyglot rejection. Explicit one-to-eight-source enrollment now freezes only selected ready imports into a new current-Saga Loom turn and the E6–E10 review path; it never invokes the E5 new-Saga commit, ordinary retrieval, embedding, or canon writes. Normal Library detail also edits bounded status/tags through the optimistic source/audit boundary and refreshes retrieval on metadata-only changes. The remaining G1 order is private image attachments/text context, then GM-authored Stage references.
...
Immediate delivery packet: continue **G1 — Import, attachments, rules, and manual closure** with private image attachments and text-only Loom context; trusted PDF intake, explicit selected-source Loom enrollment, and Library tag/status editing are complete. In parallel only when external authentication is available, close G0's Supabase dashboard toggle and private web deployment. Phases E and F are closed at their documented gates. Raw imports remain excluded from ordinary retrieval and AI unless explicitly frozen into a selected-source Loom turn. The user-authorized temporary testing credential remains installed only in ignored, access-restricted local provider state and is governed by the cumulative `$4` provider-cost stop; it must never enter source control, logs, reports, or browser state.
