...
- **G1 complete (2026-08-05):** private PDF intake, selected-source Loom enrollment, bounded Library status/tags, and private JPEG/PNG/WebP attachment/text-only Loom context are complete and deployed, with the documented local/hosted fixture matrices passing. The Stage `GM Screen` now reads only GM-authored Notes pinned during Prep; built-in D&D-like examples, system-rule search claims, and advantage/disadvantage controls are removed. Dice is a system-neutral pool/modifier convenience tool and explicitly does not adjudicate results. None of the intake/edit operations automatically invoke AI, embed, draft, or write canon.
...
Immediate delivery packet: close G0's Supabase dashboard toggle and private web deployment when external authentication is available, then run **G2 — twin real-Loom integrated journeys** under the cumulative `$4` provider-cost stop. G1, Phases E, and Phase F are closed at their documented gates. Raw imports remain excluded from ordinary retrieval and AI unless explicitly frozen into a selected-source Loom turn. The user-authorized temporary testing credential remains installed only in ignored, access-restricted local provider state and must never enter source control, logs, reports, or browser state.
