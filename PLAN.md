...
- **Phase E hosted-gate harness (ready, zero-call verified 2026-08-04):** the manifest-only preflight pins six active contracts across `relic-fast`, `relic-balanced`, and `relic-deep` for 28 product credits, at most twelve provider completions, and a `$1.00` model/provider-cost ceiling enforced by a one-concurrency temporary proxy with a `$0.99` global budget. Temporary Fly build/machine usage is separately billable and not hard-capped, although the app is destroyed in `finally`. Nine safety tests plus a real local fixture proof validate the active six-run/19-action registry, zero-provider dry run, pending-draft boundary, queue-before-provider isolation, exact replay accounting, safe telemetry, cleanup, and restoration. No staging mutation or paid/provider call was made.

**Milestone E gate:** the E5.1-E10 deterministic boundary and zero-call hosted harness are complete. Every implemented action has passed clean-replay scope, authority, idempotency, provenance, cost, canon, archive, and delete-negative coverage. The gate remains open only for explicit authorization to execute the prepared staging smoke; Phase F stays closed until that authorization decision and resulting evidence are recorded.
...
Immediate decision gate: **Phase E hosted task-family smoke authorization**. E5.1-E10 and the zero-call harness are complete. The prepared command targets `relic-staging` with six synthetic task runs, 28 product credits, `relic-fast`/Luna, `relic-balanced`/Terra, `relic-deep`/Sol, at most one repair per run and twelve total provider completions, a temporary one-concurrency proxy capped at `$0.99`, and a `$1.00` model/provider stop. Temporary Fly build/machine minutes are additional, not hard-capped, and minimized by guaranteed app destruction. The runner pauses and verifies idle AI/embedding queues before the provider swap, restores the prior provider/JWT/schedules, destroys the proxy, and proves zero fixture residue. Do not execute it until the user freshly approves this exact envelope. After an authorized passing smoke, close Milestone E and begin **F1 — Settings, retention, and quota recovery**. D5 imported sources remain excluded unless a later explicit GM enrollment contract is added.
