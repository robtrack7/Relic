...
- **Remaining gate:** Decision Stop 4 is authorized at a hard cumulative `$0.50` ceiling and the zero-inference staging fixture/auth/RLS preflight passes. The isolated hosted run is still environment-gated because the protected OpenAI key returns `401 missing_scope` for Chat Completions before a valid Guide completion. Install a staging key with Chat Completions write permission and `gpt-5.6-terra` access, then rerun the same bounded harness. Do not mark E3 complete or begin E4 until the three-turn cited/proposal/no-answer, exact-replay, metering, privacy, isolation, and zero-canon proof passes.
...
Immediate next gate: **replace or widen the staging provider key, then rerun the already-authorized isolated E3 hosted Guide smoke**. E3 is functionally complete locally; its fixture/auth/RLS preflight passes, but the provider key lacks Chat Completions write scope. E4 must not begin until the hosted gate passes. D5 imported sources remain excluded unless a later explicit GM enrollment contract is added.
