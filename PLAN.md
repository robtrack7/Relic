...
- Build deterministic adversarial text/Markdown/PDF and JPEG/PNG/WebP fixtures, including a true PDF polyglot, without making a provider call.
...
- **Checkpoint (2026-08-05):** hosted schema, exact privilege audit, advisor hardening, and deterministic 18-file fixtures are green. The gate remains open only on the authenticated Supabase leaked-password toggle and an authenticated/linked private web deployment.

#### Packet G1 — Import, attachments, rules, and manual closure

- Ship trusted text-bearing PDF extraction, immutable provenance, explicit source enrollment into a current-Saga Loom turn, and safe retry/rejection states. Keep `.docx` deferred.
...
- **Import, metadata, and private-image slices complete (2026-08-05):** private Storage PDF upload, JWT-verified extraction, immutable hashes/provenance, safe lifecycle/UI recovery, and exact RPC/Storage boundaries are deployed. Ten local and hosted PDF cases pass, including a true PDF polyglot rejection. Explicit one-to-eight-source enrollment freezes only selected ready imports into a new current-Saga Loom turn and the E6–E10 review path. Normal Library detail edits bounded status/tags through the optimistic source/audit boundary. Bounded private JPEG/PNG/WebP attachments now validate through the real Edge/Storage lifecycle; six local and hosted cases pass, short-lived viewing and object-first deletion work, and an explicit Loom handoff freezes only GM-authored description plus safe metadata while disclosing that pixels were not inspected. None of these intake/edit operations automatically invoke AI, embed, draft, or write canon. The remaining G1 item is GM-authored Stage references.
...
Immediate delivery packet: finish **G1 — Import, attachments, rules, and manual closure** by replacing Stage-owned rules examples with GM-authored reference Notes/pins; trusted PDF intake, explicit selected-source Loom enrollment, Library tag/status editing, and bounded text-only private-image attachments are complete and deployed. In parallel only when external authentication is available, close G0's Supabase dashboard toggle and private web deployment. Phases E and F are closed at their documented gates. Raw imports remain excluded from ordinary retrieval and AI unless explicitly frozen into a selected-source Loom turn. The user-authorized temporary testing credential remains installed only in ignored, access-restricted local provider state and is governed by the cumulative `$4` provider-cost stop; it must never enter source control, logs, reports, or browser state.
