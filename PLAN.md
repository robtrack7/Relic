...
- **Import slice complete (2026-08-05):** private Storage PDF upload, JWT-verified extraction, immutable hashes/provenance, safe lifecycle/UI recovery, and exact RPC/Storage boundaries are deployed. Ten local and hosted runtime cases pass, including a true PDF polyglot rejection. Explicit one-to-eight-source enrollment now freezes only selected ready imports into a new current-Saga Loom turn and the E6–E10 review path; it never invokes the E5 new-Saga commit, ordinary retrieval, embedding, or canon writes. Clean replay plus 1,198 database assertions, 156 web tests, and 105 runtime tests pass. The remaining G1 order is private image attachments/text context, Library tag/status editing, then GM-authored Stage references.
