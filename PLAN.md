...
- **PDF slice complete (2026-08-05):** private Storage upload, JWT-verified service extraction, immutable hashes/provenance, safe lifecycle/UI recovery, and exact RPC/Storage boundaries are deployed. Ten local and hosted runtime cases pass, including a true PDF polyglot rejection; all 1,181 database assertions and 155 web tests are green. The remaining G1 order is selected-source Loom enrollment, private image attachments/text context, Library tag/status editing, then GM-authored Stage references.
